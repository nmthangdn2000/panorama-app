...
export type ProjectPanoramaApi = {
  ...
  renderProject: (name: string, size: number, data: RenderProject) => Promise<boolean>;
...
const renderProject = (name: string, size: number, data: RenderProject) => ipcRenderer.invoke(KEY_IPC.RENDER_PROJECT, name, size, data);
